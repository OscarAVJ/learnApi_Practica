...
public class AuthController {
    ...
    @PostMapping("/login")
    private ResponseEntity<Map<String,String>>logIn(@Valid @RequestBody UserDTO data, HttpServletResponse response){
        if (data.getCorreo() == null || data.getCorreo().isBlank() ||
                data.getContrasena() == null || data.getContrasena().isBlank()) {
            return ResponseEntity.status(404).body(Map.of("status", "Credenciales inexistentes"));
        }

        if(service.LogIn(data.getCorreo(), data.getContrasena())){
            addTokenCookie(response, data.getCorreo());
            return ResponseEntity.ok().body(Map.of("status", "Inicio de sesion exitoso"));
        }
        return ResponseEntity.status(404).body(Map.of("status", "Credenciales incorrectas"));
    }
    private void addTokenCookie(HttpServletResponse response, String correo) {
        // Obtener el usuario completo de la base de datos
        Optional<UserEntity> userOpt = service.getUser(correo);
